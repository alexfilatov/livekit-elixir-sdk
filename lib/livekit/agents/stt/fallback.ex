defmodule Livekit.Agents.STT.Fallback do
  ...
  def stream(%Config{} = config) do
    ...
    if function_exported?(primary_mod, :stream, 1) do
      stt_stream_with_primary_fallback(primary_mod, primary_cfg, secondary_mod, secondary_cfg)
    else
      Logger.warning(
      ...
      stt_stream_via_secondary(secondary_mod, secondary_cfg, :no_provider_supports_streaming)
    end
  end

  defp stt_stream_with_primary_fallback(primary_mod, primary_cfg, secondary_mod, secondary_cfg) do
  ...
  defp stt_stream_via_secondary(secondary_mod, secondary_cfg, error_atom) do
